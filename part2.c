...
void execute_load(Instruction instruction, Processor *processor, Byte *memory) {
    ...
    switch (instruction.itype.funct3) {
        case 0x0:
            // LB
            processor->R[rd] = sign_extend_number(load(memory, rs1 + (8 * offset), LENGTH_BYTE), 8);
            break;
        case 0x1:
            // LH
            processor->R[rd] = sign_extend_number(load(memory, rs1 + (8 * offset), LENGTH_HALF_WORD), 16);
            break;
        case 0x2:
            // LW
            processor->R[rd] = load(memory, rs1 + (8 * offset), LENGTH_WORD);
            break;
        default:
            handle_invalid_instruction(instruction);
            break;
